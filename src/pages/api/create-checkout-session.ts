 ...
 *
 * Protected by: Cloudflare Turnstile (anti-bot) + Rate Limiting
 */
...
export const POST: APIRoute = async ({ request, url }) => {
  try {
    ...
    const rateLimit = checkRateLimit(clientIp, RATE_LIMITS.JOB_POST);
    ...
    if (!rateLimit.allowed) {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: `Muitas tentativas. Tente novamente em ${minutesUntilReset} minutos.`,
        }),
        {
          status: 429,
          headers: {
    ...
    const { draft_id, draft_data, turnstileToken } = body;

    // 1. TURNSTILE VERIFICATION - Anti-bot protection
