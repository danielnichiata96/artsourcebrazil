...
- 2025-11-19: **i18n Completo - Homepage PT-BR e EN**: Tradução completa da homepage usando i18n como fonte única de verdade. Adicionadas todas as strings faltantes no `i18n.ts` (hero.titleLine1/2/3, timeAgo, location labels, filters.discipline, hiringTitle/Subtitle, etc.). Página PT-BR (`index.astro`) atualizada para usar i18n (antes estava hardcoded em inglês). Página EN (`en/index.astro`) agora usa todas as traduções do i18n. Helpers JS (timeAgo, getLocationLabel) agora recebem mensagens do i18n via `define:vars`. Hero badge, título, descrição, sidebar, empty state e todos os textos agora vêm do i18n. Build passando, todas as páginas traduzidas corretamente. Commit: 8e96270.
...
- 2025-11-19: **Navbar & Footer Canvas Refresh**: `Navbar.astro` agora replica o bloco “Br” com tipografia display, borda grossa e CTA amarelo→azul mantendo toggle EN/PT e links dinâmicos via i18n. O footer passou a ter CTA de contratação, grid minimalista de links/feeds e assinatura “Made in Brazil”, reaproveitando o novo design tokens e preservando links legais e RSS.
- 2025-11-19: **Job Details Page Canvas Refresh**: Implementado novo componente `JobHeader.astro` seguindo fielmente o design system Canvas (bordas coloridas por categoria, logo em box branco, tipografia display). Criado `JobModal.astro` como shell reutilizável. Página de detalhes de vaga (`/jobs/[id]-[slug]`) atualizada para usar o novo header, garantindo consistência visual com os cards da home. Corrigido erro de build em `SearchWithAutocomplete.astro` (substituído token inválido `accent` por `primary`).
- 2025-11-19: **Homepage Canvas Sync – Phase A**: Atualizado o design system para alinhar com o layout do Gemini Canvas mantendo os fluxos existentes. Paleta: `accent.lime` agora usa Amarelo Brasil (#F7DD00) e estilos globais de checkbox refletem o novo tom. Componente `JobCard.astro` recebeu logotipo compacto da empresa, cores determinísticas por categoria e link direto para `/jobs/[id]-[slug]`, preservando a página de detalhes. Atributos extras (`data-testid`, etc.) agora são propagados automaticamente pelo card para manter a suíte de testes Playwright funcionando.
