#!/usr/bin/env node
/**
 * Sync Ashby jobs to Supabase
 * 
 * This script:
 * 1. Reads jobs from ashby-jobs-output.json
...
const ashbyJobsPath = resolve(__dirname, 'ashby-jobs-output.json');
 ...
 * Main sync function
 */
async function syncAshbyJobs() {
  console.log('🚀 Syncing Ashby Jobs → Supabase');
  console.log('═'.repeat(60));
  
  // Read jobs file
  console.log(`📖 Reading jobs from: ${ashbyJobsPath}`);
  let jobs;
  try {
    const fileContent = readFileSync(ashbyJobsPath, 'utf-8');
    jobs = JSON.parse(fileContent);
  } catch (error) {
    console.error('❌ Error reading Ashby jobs file:', error.message);
    console.error('   Make sure to run: npm run fetch:ashby first');
    process.exit(1);
  }
  ...
  console.log(`\n🔍 Verified: ${count} active Ashby jobs in Supabase\n`);
}

// Run sync
syncAshbyJobs().catch(error => {
